...
const obtenerFechasTorneo =
  'SELECT * FROM fechas WHERE id_torneo = $1 ORDER BY fecha_inicio DESC';

const obtenerFechasDelMes =
  'SELECT f.id AS id, f.nombre AS nombre_fecha, t.titulo AS titulo_torneo, f.fecha_inicio AS fecha_inicio, f.fecha_finalizacion AS fecha_finalizacion, c.nombre AS club, t.imagen_torneo AS imagen_torneo, c.direccion AS direccion, f.slug AS slug FROM fechas as f INNER JOIN torneos AS t ON t.id = f.id_torneo LEFT JOIN clubes AS c ON c.id = f.id_club WHERE (f.fecha_inicio >= $1 AND f.fecha_inicio < $2) OR (f.fecha_finalizacion >= $1 AND f.fecha_finalizacion < $2) ORDER BY f.fecha_inicio';
